...
import eu.europa.ec.cipa.etrustex.integration.util.AmqpSenderService;
...
public class ETrustEXSoapInboundGateway extends SimpleWebServiceInboundGateway implements MessageEndpoint {
	...
	public void invoke(MessageContext messageContext) {
		...
		try {
			...
			soapService.validateSOAPMessage(request, !readService, payloadRootQName);
			...
			MessageHeaders messageHeaders = DocumentQueueMessageListener.getJmsHeaders() != null ?
					DocumentQueueMessageListener.getJmsHeaders() : AmqpQueueMessageListener.getAmqpHeaders();
...
//				throw e;
			...
			//Element payload = (Element)reply.getPayload();
			Document payload = reply.getPayload();

			SaajSoapMessage response = (SaajSoapMessage) messageContext.getResponse();
			if (reply.getBinaries() != null && reply.getBinaries().size()>0 ){
				for (TrustExMessageBinary binary : reply.getBinaries()) {
					InputStreamSource source = new ByteArrayResource(binary.getBinaryContent());
					String contentId = (binary.getFileName()== null) ? UUID.randomUUID().toString() : binary.getFileName();
					response.addAttachment(contentId,source,binary.getMimeType());
				}	
			}
			...
			logger.error(e.getMessage(), e);
			JmsHeaders jmsHeaders = DocumentQueueMessageListener.getJmsHeaders() != null ?
					DocumentQueueMessageListener.getJmsHeaders() : null;
			try {
	...
	private void addFaultToResponse(MessageContext messageContext, MessageHeaders messageHeaders, String incomingNameSpace,
		...
		if (exception.getFaultDetailResponseCode() != null && StringUtils.isNotEmpty(exception.getFaultDetailResponseCode().getCode())) {
			...
			if (ErrorResponseCode.DOCUMENT_ALREADY_EXISTS.equals(exception.getFaultDetailResponseCode()) 
					|| StringUtils.isNotEmpty(exception.getFaultDetailDescription())) {
