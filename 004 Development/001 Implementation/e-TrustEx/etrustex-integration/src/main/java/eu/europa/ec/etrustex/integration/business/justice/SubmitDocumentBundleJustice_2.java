...
//			}
			...
			SequenceIterator<?> wrapperNodesIterator = queryForNode("//*:DocumentBundle/*:DocumentWrapperReference/*:ID",docInfo, config);
			List<Message> messagesToUpdate = new ArrayList<Message>();
			List<DocumentWrapperDTO> wrappersIDs = new ArrayList<>();
			while (true) {
				NodeInfo wrapperIdNode = (NodeInfo)wrapperNodesIterator.next();
