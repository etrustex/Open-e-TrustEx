...
import ec.schema.xsd.deletedocument_3.DeleteDocumentType;
...
public class DeleteDocument_3 extends TrustExBusinessService implements ISynchBusinessService {
	...
	public TrustExMessage processMessage(TrustExMessage<String> message) throws BusinessException, TechnicalErrorException {
		...
		try {
			...
			String documentId = deleteDocument.getDocumentReference().getInstanceIdentifier().trim();
			String documentTypeCode = deleteDocument.getDocumentReference().getType() != null 
					? deleteDocument.getDocumentReference().getType() : null;
			...
			String senderPartyId = StringUtils.isNotBlank(deleteDocument.getSenderParty().getIdentifier().getValue())
					? deleteDocument.getSenderParty().getIdentifier().getValue().trim() : "";
			String senderPartySchemeId = StringUtils.isNotBlank(deleteDocument.getSenderParty().getIdentifier().getAuthority()) 
					? deleteDocument.getSenderParty().getIdentifier().getAuthority().trim() : "";
			String receiverPartyId = StringUtils.isNotBlank(deleteDocument.getReceiverParty().getIdentifier().getValue())
					? deleteDocument.getReceiverParty().getIdentifier().getValue().trim() : "";
			String receiverPartySchemeId = StringUtils.isNotBlank(deleteDocument.getReceiverParty().getIdentifier().getAuthority())
					? deleteDocument.getReceiverParty().getIdentifier().getAuthority().trim() : "";
