...
public class SubmitBRISDocument_2  extends TrustExBusinessService implements IASynchBusinessService {
		
	private static final Logger logger = LoggerFactory.getLogger(SubmitBRISDocument_2.class);
	...
	public TrustExMessage<String> processMessage(TrustExMessage<String> message)
		...
		try {
			...
			final XQueryExpression exp = sqc.compileQuery("/*:BRISDocument/*:DocumentWrapperReference/*:ID");
			...
			while (true) {
				...
				if (wrappersIDs.contains(wrapperDocId)){
					throw new BusinessException( "soapenv:Client","BRISDocument Reference must be unique", null,ErrorResponseCode.DOCUMENT_BUNDLE_DOCUMENT_REFERENCE_MUST_BE_UNIQUE,doctypeCode ,ErrorResponseCode.DOCUMENT_BUNDLE_DOCUMENT_REFERENCE_MUST_BE_UNIQUE.getDescription());
				}else {
				...
				if(msg == null ){
					throw new BusinessException( "soapenv:Client","Trying to Link unexisting wrapper to a BRISDocument", null,ErrorResponseCode.DOCUMENT_BUNDLE_NON_EXISTING_DOCUMENT_REFERENCE,doctypeCode ,ErrorResponseCode.DOCUMENT_BUNDLE_NON_EXISTING_DOCUMENT_REFERENCE.getDescription());
				}
