...
import ec.schema.xsd.commoncomponents_3.DocumentReferenceResponseParentDocsType;
import ec.schema.xsd.commoncomponents_3.DocumentTypeCodeType;
import ec.schema.xsd.commoncomponents_3.RequestDocumentParameterType;
import ec.schema.xsd.queryrequest_3.QueryRequestType;
import ec.schema.xsd.queryresponse_3.ObjectFactory;
import ec.schema.xsd.queryresponse_3.QueryResponseType;
...
public class QueryRequestService_3 extends TrustExBusinessService implements ISynchBusinessService{
	
	private static final Logger logger = LoggerFactory.getLogger(QueryRequestService_3.class);
...
//				}
				...
				List<Partner> senders= requestParameter.getSenderParty();
				// we do take into account only valid party ids
				List<Partner> receivers= requestParameter.getReceiverParty();
				...
				for (Partner partyType : senders) {
					try {
						Party p =authorisationService.getParty(partyType.getIdentifier().getValue(), message.getHeader().getIssuer().getBusinessDomain());
				...
				for (Partner partyType : receivers) {
					try {
						Party p =authorisationService.getParty(partyType.getIdentifier().getValue(), message.getHeader().getIssuer().getBusinessDomain());
			...
			for (Message msg : queryResults) {
				...
				if ( ! DOC_WRAPPER_DOC_NM.equalsIgnoreCase(msg.getTransaction().getDocument().getName())){
					...
					docParentRef.setDocumentReferenceResponse(populateDocumentReferenceResponseType_3(msg));
					...
					for (Message parentMsg : parent)  {
						if ( ! DOC_WRAPPER_DOC_NM.equalsIgnoreCase(parentMsg.getTransaction().getDocument().getName())){
							docParentRef.getParentDocument().add(populateDocumentReferenceResponseType_3(parentMsg));
						}
					}
					counter++;
