...
public class SchematronServiceActivator extends XmlProcessingServiceActivator
    ...
    private String extractSchematronResults(XQueryExpression extractQuery,
        ...
        final SequenceIterator<?> iter = extractQuery.iterator(context);
    ...
    public Message<TrustExMessage<String>> processASynchMessage(
        ...
        BusinessDomain bd = partyService.getParty(message.getPayload().getHeader().getSenderPartyId()).getBusinessDomain();
        ...
        Map<MetaDataItemType, MetaDataItem> metadata2 = null;
        try {
            metadata2 = metadataService.retrieveMetaDataCheckingDuplicates(t, bd);
        } catch (DuplicateMetadataException e) {
            String msg = soapErrorMessages.getProperty(e.getMessage());
            logger.error(msg, e);
            logDTO.setDescription(logDTO.getDescription() + " " + msg);
            ...
            throw new MessageProcessingException("soapenv:Client", ErrorResponseCode.HARD_BUSINESS_RULE_VIOLATION.getDescription(), null,
                    ErrorResponseCode.HARD_BUSINESS_RULE_VIOLATION, null, msg);
        }

        // "/*:schematron-output/*:failed-assert[@flag='error']"
        final String error_xpath = metadata2.get(MetaDataItemType.SCHEMATRON_ERROR_XPATH).getValue();
...
//					messageService.updateMessage(msgDB);
    ...
    public Message<TrustExMessage> processSynchMessage(Message<TrustExMessage<String>> message) {
        ...
        } catch (TemplateException | IOException | TransformerException e) {
            logger.error(e.getMessage(), e);
            logDTO.setDescription(logDTO.getDescription() + " " + e.getMessage());
            logService.saveLog(logDTO);
