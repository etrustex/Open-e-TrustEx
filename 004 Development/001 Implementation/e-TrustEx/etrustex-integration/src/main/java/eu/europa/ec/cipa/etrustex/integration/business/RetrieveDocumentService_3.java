...
import ec.schema.xsd.commoncomponents_3.DocumentContentType;
import ec.schema.xsd.commoncomponents_3.IssueDateType;
import ec.schema.xsd.retrieverequest_3.RetrieveRequestType;
import ec.schema.xsd.retrieveresponse_3.ObjectFactory;
import ec.schema.xsd.retrieveresponse_3.RetrieveResponseType;
...
public class RetrieveDocumentService_3 extends TrustExBusinessService implements
	...
	public TrustExMessage<JAXBElement> processMessage(
		...
		try {
			...
			if (StringUtils.isNotEmpty(request.getSenderParty().getIdentifier().getAuthority())) {
				partyIdWithScheme += request.getSenderParty().getIdentifier().getAuthority() + "#";
			}
			partyIdWithScheme += request.getSenderParty().getIdentifier().getValue();
			try {				
			...
			partyIdWithScheme = "";
			if (StringUtils.isNotEmpty(request.getReceiverParty().getIdentifier().getAuthority())) {
				partyIdWithScheme += request.getReceiverParty().getIdentifier().getAuthority() + "#";
			}
			partyIdWithScheme += request.getReceiverParty().getIdentifier().getValue();
			...
			if ((receiverParty != null  && receiverParty.getId().equals(message.getHeader().getSenderPartyId())) || 
				...
				String messageDocumentId = request.getDocumentReference().getInstanceIdentifier();
				...
				if (queryResult != null && queryResult.size() >0) {
					response.setSenderParty(populateParty_3(senderParty.getIdentifiers()));
					response.setReceiverParty(populateParty_3(receiverParty.getIdentifiers()));
					Message retrieved = queryResult.get(0);
					IssueDateType receptionDate = new IssueDateType();
					...
					response.setIssueDate(receptionDate);
