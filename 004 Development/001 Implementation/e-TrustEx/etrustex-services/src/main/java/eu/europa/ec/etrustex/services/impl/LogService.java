...
public class LogService extends ThreadLocal<String> implements ILogService {
	...
	public LogDTO saveLog(LogDTO logDTO) {
		...
		Log	log = new Log.LogBuilder(logDTO.getLogType().name(), logDTO.getCorrelationId())
			...
			.documentId(StringUtils.isNotBlank(logDTO.getDocumentId()) && logDTO.getDocumentId().length() > 250 
					? logDTO.getDocumentId().substring(0, 250)
					: logDTO.getDocumentId())
	...
	public int removeBinarySizeInLogs(String correlationId) {
		return logDAO.removeBinarySizeInLogs(correlationId);
	}
}
