...
//		}		
	...
	public static EncryptBinaryDTO encryptWithAES(InputStream in, OutputStream out, byte[] key, int bufLength, 
			boolean enforceMaxNumberOfBytes) throws BinaryStreamLimitExceededException {
        ...
        try {
            ...
            copyFromSourceToOutput(in, out, bufLength, enforceMaxNumberOfBytes);
        }
	...
	public static void copyFromSourceToOutput(InputStream in, OutputStream out, int bufLength, 
			boolean enforceMaxNumberOfBytesLimit) throws IOException, BinaryStreamLimitExceededException {
        ...
        while ((numRead = in.read(buf)) >= 0){
        	if (enforceMaxNumberOfBytesLimit && numberOfByte > MessageBinaryDAO.STREAM_MAX_NUMBER_OF_BYTES) {
        		out.flush();
        		out.close();
				throw new BinaryStreamLimitExceededException(
						"Read " + numberOfByte + " bytes. Max " + MessageBinaryDAO.STREAM_MAX_NUMBER_OF_BYTES + " bytes can be submitted.");
			}
        	out.write(buf, 0, numRead);
        	numberOfByte = numberOfByte + numRead;
        	if (numRead == 1) {
        		break;
        	}
        }
