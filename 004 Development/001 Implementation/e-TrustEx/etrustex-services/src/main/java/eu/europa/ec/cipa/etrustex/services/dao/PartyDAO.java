...
import eu.europa.ec.cipa.etrustex.domain.PartyIdentifier_;
...
public class PartyDAO extends TrustExDAO<Party, Long> implements IPartyDAO {
    private static final String EVENT_NOTIF_PUBLISHER_ETRUSTEX = "ETRUSTEX";
	...
	public Party getParty(IIdentifierIssuingAgency shemeId, String partyId, BusinessDomain bd) {
		...
		return entityManager
				.createQuery("select id.party from PartyIdentifier id where upper(id.value) = :id and id.schemeId = :scheme and id.party.businessDomain.id = :bdid",Party.class)
				.setParameter("id", partyId.trim().toUpperCase())
				.setParameter("scheme", shemeId)
				.setParameter("bdid", bd!=null?bd.getId():null).getSingleResult();
	}
...
//		return getUniqueByAttribute(Party_.name, name);
    ...
    public List<Party> findPartiesByCriteria(String name, String username, String identifierValue, List<Long> businessDomainIds, Boolean isThirdParty) {
        ...
        Join<Party, PartyIdentifier> identifiers = party.join(Party_.identifiers);
        List<Predicate> predicates = new ArrayList<Predicate>();

        if (StringUtils.isNotBlank(identifierValue)) {
            predicates.add(cb.like(cb.trim(CriteriaBuilder.Trimspec.BOTH, cb.upper(identifiers.get(PartyIdentifier_.value))), "%" + identifierValue.trim().toUpperCase() + "%"));
        }
        ...
        if (StringUtils.isNotBlank(username)) {
            predicates.add(cb.like(cb.trim(CriteriaBuilder.Trimspec.BOTH, cb.upper(party.get(Party_.credentials).get(Credentials_.user))), "%" + username.trim().toUpperCase() + "%"));
        }
	...
	public List<Role> getRoles(Party party){
		String queryString = "SELECT distinct partyRole.role FROM PartyRole partyRole where partyRole.party = :p order by UPPER(partyRole.party.name) ";
		TypedQuery<Role> query = entityManager.createQuery(queryString, Role.class);
		query.setParameter("p", party);
...
//		}
	...
	public List<PartyListItemDTO> getAllPartiesForBusinessDomain(Long businessDomainId) {
		...
		return entityManager
				.createQuery("select new eu.europa.ec.cipa.etrustex.services.dto.PartyListItemDTO(p.id, p.name) from Party p " +
    ...
    public List<PartyListItemDTO> getNonThirdPartiesForBusinessDomain(Long businessDomainId) {
        return entityManager
                .createQuery("select new eu.europa.ec.cipa.etrustex.services.dto.PartyListItemDTO(p.id, p.name) from Party p " +
                        "where (p.thirdPartyFlag = null or p.thirdPartyFlag = 0) and p.businessDomain.id = :businessDomainId " +
                        "order by UPPER(p.name) ", PartyListItemDTO.class)
                .setParameter("businessDomainId", businessDomainId)
                .getResultList();
    }
	...
	public Party getEventPublisherParty(Party confSubscriberParty) {
		StringBuilder stringBuilder = new StringBuilder()
			...
			.append("and pr1.PAR_ROL_ID = :eventSubscriberRoleId ") //subscriber must have EventurationSubscriber role
