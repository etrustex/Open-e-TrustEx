var p = {
    ...
    $._AE('change', '#credentialsPasswordEncrypted_bs', function() {
      if ( $('#credentialsPasswordEncrypted').val() === 'true' && $("#pageMode").val() === 'edit' && $.trim($('#credentialsUser').val()).length !== 0 && $('#credentialsPassword1').val().length === 0) {
        $._msgbox({
          dialogId:'confirmMsg',
          title: $._getData('party.management.decrypt.message.title'),
          text: $._getData('party.management.decrypt.message'),
          msgboxType:'ok',
          alertType:'warning',
