...
import eu.europa.ec.etrustex.admin.old.web.dto.CredentialsForm;
...
import eu.europa.ec.etrustex.admin.old.web.utils.MsgConverterClassEnum;
import eu.europa.ec.etrustex.admin.old.web.utils.Referer;
import eu.europa.ec.etrustex.admin.old.web.utils.UserRoleEnum;
import eu.europa.ec.etrustex.admin.old.web.validators.EndpointSearchValidator;
import eu.europa.ec.etrustex.admin.old.web.validators.EndpointValidator;
import eu.europa.ec.etrustex.admin.old.web.validators.ValidationHelper;
import eu.europa.ec.etrustex.dao.dto.CredentialsDTO;
...
public class EndpointController {
    ...
    @Autowired
    private EndpointSearchValidator endpointSearchValidator;
    @Autowired
    ...
    public AjaxResult save(@ModelAttribute("endpointForm") @Valid EndpointForm form, BindingResult result, @PathVariable("configurationType") String configurationType,
                           @ActiveUser SessionUserInformation userInfo, HttpServletRequest request) throws UnsupportedEncodingException, GeneralSecurityException {
    ...
    public String viewLoad(@PathVariable("endpointId") Long endpointId, @ModelAttribute("endpointSearchForm") EndpointSearchForm endpointSearchForm,
        ...
        if ( endpoint.getCredentials() != null ) {
            endpoint.setCredentials(credentialsService.get(endpoint.getCredentials().getId()));
        }

        if ( endpoint.getProxyCredential() != null ) {
            endpoint.setProxyCredential(credentialsService.get(endpoint.getProxyCredential().getId()));
        }
...
//		BeanUtils.copyProperties(endpoint, form, ignoredProps); 
    ...
    public String credentialsDialog(@RequestParam(value = "isSearch", required = false) boolean isSearch, @RequestParam boolean isProxyCredentials, @RequestParam(value = "credentialsId", required = false) Long credentialsId,
        ...
        if (credentialsId != null) {
            ...
            if (credentials != null) {
                String password = decryptPassword(credentials);
                credentialsForm.setId(credentials.getId());
                credentialsForm.setUsername(credentials.getUser());
                credentialsForm.setPassword(password);
                credentialsForm.setPassword2(password);
            }
        }
    ...
    public String credentialsLoad(@RequestParam boolean isProxyCredentials, @RequestParam(value = "credentialsId", required = false) Long credentialsId,
        ...
        Long credId = credentialsId != null ? credentialsId : isProxyCredentials ? endpointForm.getProxyCredential().getId() : endpointForm.getCredentials().getId();
        if (credId != null) {
            Credentials credentials = credentialsService.get(credId);
            if( credentialsForm.getPassword() !=  null ){
                user = credentialsForm.getUsername();
                password = credentialsForm.getPassword();
            }else {
                user = credentials.getUser();
                password = decryptPassword(credentials);
            }
    ...
    private ConfigurationTypeEnum getConfigurationType(Endpoint endpoint) {
        if (endpoint instanceof JMSEndpoint) {
            return ConfigurationTypeEnum.JMS;
    ...
    private String getConfigurationTypeAsString(Endpoint endpoint) {
        if (endpoint instanceof JMSEndpoint) {
            return ConfigurationTypeEnum.JMS.toString();
        } else if (endpoint instanceof WSEndpoint) {
    ...
    private String decryptPassword(Credentials cred){
