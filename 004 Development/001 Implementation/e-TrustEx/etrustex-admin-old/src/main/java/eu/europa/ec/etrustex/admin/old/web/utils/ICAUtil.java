...
public class ICAUtil {
     ...
     */
    public void validateProfile(ProfileElement profileEl, SessionUserInformation userInfo, Errors errors) {
        BusinessDomain businessDomain = businessDomainService.getBusinessDomain(userInfo.getBusinessDomain().getId());
        if (profileEl == null || FormUtil.convertDefaultOptionToNull(profileEl.getId()) == null) {
            ...
            Set<Profile> profiles = businessDomain.getProfiles();
     ...
     * - cannot be in the past for ICA creation. (originalDate = null)
     * - cannot be in the past for ICA modification if the date is changed.
     */
    public void validateStartDate(Date date, Date originalDate, Errors errors) {
        if (date != null) {
            ...
            if (startDate.before(today)) {
                ...
                } else if (!ObjectUtils.equals(LocalDate.fromDateFields(date), LocalDate.fromDateFields(originalDate))) {
                    errors.rejectValue("icaValidityStartDate", "error.ica.edit.validFrom.todayOrFutureDate");
                }
            }
        }
    }
    ...
    public List<SelectElement> getPartyDelegates(Long partyId) {
        List<SelectElement> partyElements = new ArrayList<>();

        for (Party party : partyService.getAllThirdPartiesFor(partyId)) {
            partyElements.add(new SelectElement(party.getId(), party.getName()));
        }
