...
public class PartyController {
    ...
    public AjaxResult save(@ModelAttribute("partyForm") @Valid PartyView partyView, BindingResult result,
        ...
        if (certificatesForm != null) {
            ...
            if(CollectionUtils.isNotEmpty(certificatesForm.getCertificates())) {
            ...
            for (Long certId : certificatesForm.getCertificatesToDelete()) {
                if(certId != null){
...
//	}
    ...
    private String validateNewIdentifier(SessionUserInformation userInfo, PartyView partyView) {
        ...
        if (StringUtils.isNotBlank(partyView.getNewIdentifierType())
            ...
            //validate if EC:LEF identifier
            if(partyView.getNewIdentifierType().equals(IdentifierIssuingAgency.EC_LEF.name())
                    && (partyView.getNewIdentifierValue().length() != 10 || !partyView.getNewIdentifierValue().startsWith("600") || !partyView.getNewIdentifierValue().chars().allMatch(Character::isDigit))) {
                return messageSource.getMessage("error.party.identifier.value.eclef", null, "EC_LEF format not valid", LocaleContextHolder.getLocale());
            }
