...
public class EndpointValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        if (configurationType.equals(JMS)) {
            ...
            if(!validateUniqueCredentials(new JMSCredentials(), JMSEndpoint.class, form.getCredentials().getUser(), form.getCredentials().getId(), form.getBusinessDomain().getId(), form.getProviderUrl(), form.getId())){
            ...
            if(!validateUniqueCredentials(new WSCredentials(), WSEndpoint.class, form.getCredentials().getUser(), form.getCredentials().getId(), form.getBusinessDomain().getId(), form.getWsEndpointURL(), form.getId())){
            ...
            if(!validateUniqueCredentials(new AMQPCredentials(), AMQPEndpoint.class, form.getCredentials().getUser(), form.getCredentials().getId(),form.getBusinessDomain().getId(), form.getProviderUrl(), form.getId())){
        ...
        if (form.getUseProxy() || !StringUtils.isEmpty(proxyCredentials.getUser()) || !StringUtils.isEmpty(proxyCredentials.getPassword())) {
            ...
            }else if(!validateUniqueProxyCredentials(proxyCredentials, form.getProxyHost(), form.getProxyPort())){
    ...
    private boolean validateUniqueCredentials(Credentials credCriteria, Class<? extends Endpoint> endpointClass, String credUser, Long newCredId, Long bdId, String newUrl, Long endpointId){
        if (StringUtils.isBlank(credUser)) {
            return true;
        }

        credCriteria.setUser(credUser);
        return !endpointService.existsEndpointForUsernameAndUrl(credCriteria, bdId, endpointClass, newUrl, endpointId, newCredId);
    }

    private boolean validateUniqueProxyCredentials(Credentials proxyCredentials, String host, Integer port){
