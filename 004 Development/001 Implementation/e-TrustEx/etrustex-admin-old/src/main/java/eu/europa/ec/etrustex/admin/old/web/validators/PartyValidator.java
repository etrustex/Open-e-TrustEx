...
//	@Autowired private CertificateValidator certificateValidator;
    ...
    private void validateIdentifiers(PartyView partyView, Errors errors, SessionUserInformation userInfo) {
        ...
        if (isNewIdentifierFilledIn) {
            ...
            if(pi.getSchemeId().name().equals(IdentifierIssuingAgency.EC_LEF.name())
                    && (pi.getValue().length() != 10 || !pi.getValue().startsWith("600") || !pi.getValue().chars().allMatch(Character::isDigit))) {
                errors.rejectValue(IDENTIFIER_VALUE, "error.party.identifier.value.eclef", "EC_LEF format not valid");
            }
            ...
            } else if (pi.getValue().length() < ValidationHelper.MIN_LENGHT) {
        ...
        if (!isEmptyList) {
            for (PartyIdentifierDTO partyIdentifierDTO : partyIdentifiers) {
                ...
                if (StringUtils.isNotBlank(partyIdentifierDTO.getValue())) {
                    ...
                    if(partyIdentifierDTO.getType().name().equals(IdentifierIssuingAgency.EC_LEF.name())){
                        if(partyIdentifierDTO.getValue().length() != 10 || !partyIdentifierDTO.getValue().startsWith("600") || !partyIdentifierDTO.getValue().chars().allMatch(Character::isDigit)){
                            errors.rejectValue(IDENTIFIER_VALUE, "error.party.identifier.value.eclef", "EC_LEF format not valid");
                        }
                    }
