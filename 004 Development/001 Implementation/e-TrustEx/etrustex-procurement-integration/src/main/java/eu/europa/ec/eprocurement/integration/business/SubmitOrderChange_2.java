...
@Component("SubmitOrderChange-2.0")
public class SubmitOrderChange_2 extends TrustExBusinessService implements IASynchBusinessService {

	private static final Logger logger = LoggerFactory.getLogger(SubmitOrderChange_2.class);

	private static final String PARENT_DOC_ID_XPATH = "/*:OrderChange/*:OrderReference/*:ID";
	...
	public TrustExMessage<String> processMessage(TrustExMessage<String> message) throws BusinessException, TechnicalErrorException {
		...
		try {
			...
			Message parent = messageService.retrieveMessage(parentId, ORDER_TYPE_CODE, message.getHeader().getReceiverPartyId(), message.getHeader().getSenderPartyId(), false, null);
