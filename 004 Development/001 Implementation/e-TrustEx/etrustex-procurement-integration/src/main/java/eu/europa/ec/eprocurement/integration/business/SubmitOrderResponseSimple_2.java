...
@Component("SubmitOrderResponseSimple-2.0")
public class SubmitOrderResponseSimple_2 extends TrustExBusinessService implements IASynchBusinessService {

	private static final Logger logger = LoggerFactory.getLogger(SubmitOrderResponseSimple_2.class);

	private static final String PARENT_DOC_ID_XPATH = "/*:OrderResponseSimple/*:OrderReference/*:ID";
	private static final String INDICATOR_XPATH = "/*:OrderResponseSimple/*:AcceptedIndicator";
	...
	public TrustExMessage<String> processMessage(TrustExMessage<String> message) throws BusinessException, TechnicalErrorException {
		...
		try {
			...
			// Already Checked in Schematron, should be Boolean
			Boolean accepted = Boolean.parseBoolean(queryForSingle(INDICATOR_XPATH, docInfo, config));
			String action = (accepted ? "ACCEPT" : "REJECT");

			// Get the Parent Message: Order
			Message parent = messageService.retrieveMessage(parentId, ORDER_TYPE_CODE, message.getHeader().getReceiverPartyId(), message.getHeader()
					.getSenderPartyId(), false, null);

			// Check if the order exists and has the right state
			if (parent == null || !DocumentStatusCode.RECEIVED.name().equals(parent.getStatusCode())) {
			...
			String newStatus = smService.getNextState(parent.getTransaction().getDocument(), parent.getStatusCode(), action);
