...
@Component("SubmitOrderAcknowledgement-2.0")
public class SubmitOrderAcknowledgement_2 extends TrustExBusinessService implements IASynchBusinessService {

	private static final Logger logger = LoggerFactory.getLogger(SubmitOrderAcknowledgement_2.class);

	private static final String PARENT_DOC_ID_XPATH = "/*:OrderResponseSimple/*:OrderReference/*:ID";
	private static final String ORDER_TYPE_CODE = "220";
	private static final String STATE_PROCESSED_EVENT = "PROCESS";
	...
	public TrustExMessage<String> processMessage(TrustExMessage<String> message) throws BusinessException, TechnicalErrorException {
		...
		try {
			...
			Message parent = messageService.retrieveMessage(parentId, ORDER_TYPE_CODE, message.getHeader().getSenderPartyId(), message.getHeader().getReceiverPartyId(), false, null);

			// Check if the order exists and has the right state
			if (parent == null || !DocumentStatusCode.ACCEPTED.name().equals(parent.getStatusCode())) {
				throw new BusinessException("soapenv:Client", "Parent Document ID does not exist or has a wrong status", null, ErrorResponseCode.DOCUMENT_PARENT_ID_NOT_EXIST, doctypeCode,
						"Parent Document ID does not exist or has a wrong status");
			}

			String newStatus = smService.getNextState(parent.getTransaction().getDocument(), parent.getStatusCode(), STATE_PROCESSED_EVENT);
