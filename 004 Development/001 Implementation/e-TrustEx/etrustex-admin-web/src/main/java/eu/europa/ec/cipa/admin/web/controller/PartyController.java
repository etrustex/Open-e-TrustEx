...
import eu.europa.ec.cipa.admin.web.services.IPartyNotificationService;
import eu.europa.ec.cipa.admin.web.utils.AjaxResult;
import eu.europa.ec.cipa.admin.web.utils.PartyUtil;
import eu.europa.ec.cipa.admin.web.utils.Referer;
import eu.europa.ec.cipa.admin.web.utils.UserRoleEnum;
import eu.europa.ec.cipa.admin.web.validators.CertificateValidator;
import eu.europa.ec.cipa.admin.web.validators.PartySearchValidator;
import eu.europa.ec.cipa.admin.web.validators.PartyValidator;
...
public class PartyController {
	...
	@Autowired private IPartyNotificationService partyNotificationService;
	...
	@Autowired private PartySearchValidator partySearchValidator;
	...
	public AjaxResult save(@ModelAttribute("partyForm") @Valid PartyView partyView, BindingResult result, 
		...
		return new AjaxResult(true, messageSource.getMessage("party.success.save", new Object[] { partyId }, LocaleContextHolder.getLocale()), partyId);
	}
    ...
    public String searchLoad(@RequestParam(value="isSearchDialog", required = false) Boolean isSearchDialog,
        ...
        if(isSearchDialog != null && isSearchDialog) {
            ...
            if ( (userInfo.getRole().getCode().equals(UserRoleEnum.CBO.name()) && businessDomainId != null)
                    || ( Objects.equals(partyType, "authorizing") || Objects.equals(partyType, "delegate")) ) {
            ...
            if(StringUtils.isNotBlank(partyType)) {
                switch(partyType) {
                    case "first": case "second": case "authorizing":
                        ...
                        break;
                    case "first3rd": case "second3rd": case "delegate":
	...
	public String searchResults(@ModelAttribute("partySearchForm") @Valid PartySearchForm partySearchForm, BindingResult result,
		...
		List<SearchPartyElement> results = new ArrayList<SearchPartyElement>();
	...
	public String viewLoad(@PathVariable("partyId") Long id, @ModelAttribute("partySearchForm") PartySearchForm partySearchForm,
        ...
        if(party != null) {
            ...
            populateForm(model, new PartyView(), "view");
        }
	...
	public AjaxResult delete(@PathVariable("partyId") Long partyId, 
        ...
        /*
         * UC10_BR36	Parties that are used in an Interchange Agreement cannot be deleted. User is notified if he/she attempts to delete such party.
         ...
         */
		if(interchangeAgreementService.containsParty(party) || !messageRoutingService.findByEndpointPartyId(partyId).isEmpty()) {
			return new AjaxResult(false, messageSource.getMessage("error.party.in.use", null, LocaleContextHolder.getLocale()), party.getId());
		}

		try {
//			partyService.deleteById(Long.valueOf(partyId));
			partyNotificationService.deleteAndSendNotification(partyId, userInfo.getBusinessDomain().getId());
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			return new AjaxResult(false, messageSource.getMessage("common.system.error", null, LocaleContextHolder.getLocale()), party.getId());
		}
...
//		Party savedParty = partyService.createParty(newParty);
...
//		Party savedParty = partyService.updateParty(modifiedParty, partyIdentifiersToDelete);
