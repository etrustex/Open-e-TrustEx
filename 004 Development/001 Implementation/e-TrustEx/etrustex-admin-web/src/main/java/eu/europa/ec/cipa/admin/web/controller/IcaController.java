...
import eu.europa.ec.cipa.etrustex.services.report.xls.ICAResultXlsExportService;
...
public class IcaController {
	...
	@Autowired private ICAResultXlsExportService xlsReport;
...
//			InterchangeAgreement savedIca = interchangeAgreementService.updateInterchangeAgreement(modifiedIca, firstDelegation, secondDelegation);
	...
	public String viewLoad(@PathVariable("icaId") Long id,
		...
		if(ica != null) {
			...
			populateForm(model, new InterchangeAgreementForm(), "view", userInfo);
		}
...
//			interchangeAgreementService.deleteInterchangeAgreementById(Long.valueOf(icaId));
    ...
    public String partyDelegates(@PathVariable Long partyId, @RequestParam String partyType, @ModelAttribute("interchangeAgreement") InterchangeAgreementForm interchangeAgreement,
        ...
        if(partyType.equals("first")) {
            interchangeAgreement.setFirstPartyDelegates(getPartyDelegates(partyId));
        } else {
            interchangeAgreement.setSecondPartyDelegates(getPartyDelegates(partyId));
        }
...
//		partyForm.setVia3rdPartyName(thirdPartyName);
		...
		form.setFirstPartyDelegates(getPartyDelegates(form.getFirstParty().getId()));
		form.setSecondPartyDelegates(getPartyDelegates(form.getSecondParty().getId()));
    ...
    private List<SelectElement> getPartyDelegates(Long partyId) {
