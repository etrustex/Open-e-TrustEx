...
//        });
		...
		/* Bug filter table results */
		$._$document.on('keyup', 'input.table-filter', function () {
            var tableIdToFilter =$(this).attr('tableIdToFilter');

            /*
             * Olnly for selectable results (E.g. ICA Multiple creation
             */
             if ($('#' + tableIdToFilter + ' thead tr th').has( ":checkbox" ).length) {
                // Uncheck non visible records checkboxes
                // See https://webgate.ec.europa.eu/CITnet/jira/browse/ETRUSTEX-1308?focusedCommentId=1649026&page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel#comment-1649026
                // $('#' + tableIdToFilter + ' tbody .checkable:checked:hidden').prop("checked", false);
                ...
                $('#checkAllFlagId').prop("checked", allChecked);

                // Update table caption with count of visible records
                $('#' + tableIdToFilter + ' caption span').text($('#' + tableIdToFilter + ' tbody [type=checkbox]:visible').length);
             } else {
                // Update table caption with count of visible records
                $('#' + tableIdToFilter + ' caption span').text($('#' + tableIdToFilter + ' tbody tr:visible').length);
             }
        });
	},
