...
import eu.europa.ec.etrustex.domain.ServiceEndpoint_;
...
import eu.europa.ec.etrustex.domain.util.MessageResponseCode_;
...
public class TransactionDAO extends TrustExDAO<Transaction, Long> implements
	...
	public Transaction getTransactionByNameSpace(String transactionNameSpace,String transactionRequestLocalName){
		List<Transaction> results= entityManager.createQuery(
				"from Transaction tra where tra.namespace= :transactionNameSpace and tra.requestLocalName= :transactionRequestLocalName",Transaction.class)
				.setParameter("transactionNameSpace", transactionNameSpace)
				.setParameter("transactionRequestLocalName", transactionRequestLocalName)
				.getResultList();
	...
	public List<Transaction> getTransactionsByDocumentTypeCd(
		...
		return entityManager.createQuery(
				"select tra from Transaction tra join tra.document doc where doc.documentTypeCode= :documentTypeCode ",Transaction.class)
				.setParameter("documentTypeCode", documentTypeCode)
				.getResultList();
	}
	...
	public List<Transaction> getTransactionsForDocument(
		...
		return entityManager.createQuery(
				"select tra from Transaction tra join tra.document doc where doc.localName= :documentlocalName and doc.namespace = :documentNamespace ",Transaction.class)
				.setParameter("documentlocalName", documentlocalName)
				.setParameter("documentNamespace", documentNamespace)
				.getResultList();
	}
	...
	public List<Transaction> getTransactionsByCriteria(Transaction transaction) {
		...
		if(transaction.getDocument() != null && StringUtils.isNotEmpty(transaction.getDocument().getName())) {
			Join<Transaction, Document> docJoin = tx.join(Transaction_.document, JoinType.INNER);
			predicates.add(cb.like(cb.trim(Trimspec.BOTH, cb.upper(docJoin.get(Document_.name))), "%" + transaction.getDocument().getName().trim().toUpperCase() + "%"));
		}
...
//		Set<Profile> profiles = tx.getProfiles();
	...
	public boolean isInUse(Transaction tx) {
        ...
        cq = cb.createQuery(Long.class);
        Root<Policy> policyRoot = cq.from(Policy.class);
        cq.select(cb.count(policyRoot));
        cq.where(cb.equal(policyRoot.get(Policy_.transaction), tx));
        if (entityManager.createQuery(cq).getSingleResult() > 0) {
            return true;
        }
