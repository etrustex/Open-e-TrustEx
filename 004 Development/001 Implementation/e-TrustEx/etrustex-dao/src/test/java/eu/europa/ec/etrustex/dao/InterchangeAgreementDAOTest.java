...
public class InterchangeAgreementDAOTest extends AbstractEtrustExTest {
	...
	@Test public void testIsUniqueBy(){
		...
		Boolean boolResult = icaDao.isUniqueBy(p1.getId(), party1.getId(), role1.getId(), party2.getId(), role2.getId(), null);
		
		Assert.assertTrue(boolResult);
		
		icaDao.create(buildICA(ciaLevel6, p1, pr1, pr2));
		
		boolResult = icaDao.isUniqueBy(p1.getId(), party1.getId(), role1.getId(), party2.getId(), role2.getId(), null);
		
		Assert.assertFalse(boolResult);
		
		boolResult = icaDao.isUniqueBy(p1.getId(), party2.getId(), role2.getId(), party1.getId(), role1.getId(), null);
