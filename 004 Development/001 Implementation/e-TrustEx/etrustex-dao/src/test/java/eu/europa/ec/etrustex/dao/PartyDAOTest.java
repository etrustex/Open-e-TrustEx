...
public class PartyDAOTest extends AbstractEtrustExTest {
	...
	public void testFindPartiesByCriteriaBusinessDomain(){
		...
		List<Party> resultList = partyDao.findPartiesByCriteria(null, null, null, new ArrayList<Long>((Arrays.asList(businessDomain2.getId()))), null);
		...
		List<Party> resultList2 = partyDao.findPartiesByCriteria(PARTY_NAME, null, null, new ArrayList<Long>((Arrays.asList(businessDomain.getId()))), null);
	...
	public void testFindPartiesByCriteria(){
		...
		resultList = partyDao.findPartiesByCriteria(null, null, null, new ArrayList<Long>((Arrays.asList(businessDomain.getId()))), null);
		Assert.assertEquals("Should find 3 Parties", 3, resultList.size());
		
		resultList = partyDao.findPartiesByCriteria(PARTY_NAME_3, null, null, null, null);
		Assert.assertEquals("Did not find result by party name", 1, resultList.size());
		
		resultList = partyDao.findPartiesByCriteria(null, null, null, new ArrayList<Long>((Arrays.asList(businessDomain.getId()))), null);
		Assert.assertEquals("Should find 3 Parties", 3, resultList.size());
			
		resultList = partyDao.findPartiesByCriteria(null, null, null, new ArrayList<Long>((Arrays.asList(businessDomain.getId()))), Boolean.TRUE);
		Assert.assertEquals("Should find 2 parties with third party flag", 2, resultList.size());
		
		resultList = partyDao.findPartiesByCriteria(null, CREDENTIALS_NAME, null, null, null);
		Assert.assertEquals("Should only find one party with credentials", 1, resultList.size());
		
		resultList = partyDao.findPartiesByCriteria(null, null, PARTY_NAME_PI_3, null, null);
